...
const MainAdminPanel = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const productsResponse = await axios.get('v');
  ...
  const handleOrderApproval = (orderId, action) => {
    ...
    // Logic for handling order approval or rejection
  };

  const handlePharmacistApproval = (pharmacistId, action) => {
    console.log(`Pharmacist ${pharmacistId} action: ${action}`);
    // Logic for handling pharmacist approval or rejection
  };
          ...
          <section>
            ...
            <div className="space-y-6">
              {orders.map((order) => (
                <div
                  key={order._id}
                  className="border border-gray-200 bg-white p-4 rounded-lg shadow-md"
                >
